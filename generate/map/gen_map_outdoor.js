...
import GenRoomTunnelClass from '../../generate/map/gen_map_tunnel.js';
...
export default class GenMapOutdoorClass
{
    constructor(view,map,callbackFunc)
    {
        ...
        this.ROOM_LONG_TUNNEL_PERCENTAGE=0.3;          // what percentage of the time the general room path will have a long tunnel
        ...
        this.TUNNEL_LIGHT_INTENSITY=constants.ROOM_FLOOR_HEIGHT*1.6;                 // intensity of tunnel lights
    ...
    addRegularRoom(level,pathType,xBlockSize,zBlockSize,xBound,zBound,mainPath,mainPathSide,mainPathConnectedRoom,extensionDirection)
    {
        ...
        //
        // tunnels and liquid steps
        //
        
    addTunnelRoom(connectSide,tunnelMode,xTunnelBound,zTunnelBound)
    {
            // build the tunnel
            
        let yTunnelBound=new BoundClass(this.yBase,(this.yBase-constants.ROOM_FLOOR_HEIGHT));        // don't count the upper header

        if ((connectSide===constants.ROOM_SIDE_LEFT) || (connectSide===constants.ROOM_SIDE_RIGHT)) {
            this.genRoomTunnel.createTunnelX(xTunnelBound,yTunnelBound,zTunnelBound,(tunnelMode===this.TUNNEL_LONG));
        }
        else {
            this.genRoomTunnel.createTunnelZ(xTunnelBound,yTunnelBound,zTunnelBound,(tunnelMode===this.TUNNEL_LONG));
        }
        
            // add to overlay
            
        this.map.overlay.addConnection(xTunnelBound,zTunnelBound);
    }
    ...
    addTunnelLight(connectSide,tunnelMode,tunnelSize,xBound,zBound)
    {
        ...
        if (tunnelMode===this.TUNNEL_LONG) {
            fixturePos=new PointClass(xBound.getMidPoint(),(this.yBase-constants.ROOM_FLOOR_HEIGHT),zBound.getMidPoint());
            lightPos=new PointClass(fixturePos.x,(fixturePos.y+1100),fixturePos.z);
            this.addGeneralLight(lightPos,fixturePos,null,this.TUNNEL_LIGHT_INTENSITY,true);
        }
        ...
        if ((connectSide===constants.ROOM_SIDE_LEFT) || (connectSide===constants.ROOM_SIDE_RIGHT)) {
            xAdd=Math.trunc(tunnelSize*0.5)+constants.ROOM_FLOOR_DEPTH;
            xAdd2=1000;
            zAdd=zAdd2=0;
            ...
            xAdd=xAdd2=0;
            zAdd=Math.trunc(tunnelSize*0.5)+constants.ROOM_FLOOR_DEPTH;
            zAdd2=1000;
    ...
    buildMapRoomPath(lastRoom,tunnelMode)
    {
        ...
        if (lastRoom===null) {
            ...
            while (true) {
                ...
                connectOffset*=constants.ROOM_BLOCK_WIDTH;
                    ...
                    // if we need space for tunnels
                
                doorAdd=(tunnelMode===this.TUNNEL_LONG)?(constants.ROOM_BLOCK_WIDTH*4):constants.ROOM_BLOCK_WIDTH;
                ...
                switch (connectSide) {
                    ...
                    case constants.ROOM_SIDE_LEFT:
                        ...
                        if (tunnelMode!==this.TUNNEL_NONE) {
                            xBound.add(-doorAdd);
                            doorOffset=this.findRandomBlockOffsetBetweenTwoBounds(lastRoom.zBound,zBound);
                            xTunnelBound=new BoundClass((lastRoom.xBound.min-doorAdd),lastRoom.xBound.min);
                            zTunnelBound=new BoundClass((lastRoom.zBound.min+doorOffset),((lastRoom.zBound.min+doorOffset)+constants.ROOM_BLOCK_WIDTH));
                        }
                    ...
                    case constants.ROOM_SIDE_TOP:
                        ...
                        if (tunnelMode!==this.TUNNEL_NONE) {
                            ...
                            xTunnelBound=new BoundClass((lastRoom.xBound.min+doorOffset),((lastRoom.xBound.min+doorOffset)+constants.ROOM_BLOCK_WIDTH));
                            zTunnelBound=new BoundClass((lastRoom.zBound.min-doorAdd),lastRoom.zBound.min);
                        }
                    ...
                    case constants.ROOM_SIDE_RIGHT:
                        ...
                        if (tunnelMode!==this.TUNNEL_NONE) {
                            xBound.add(doorAdd);
                            doorOffset=this.findRandomBlockOffsetBetweenTwoBounds(lastRoom.zBound,zBound);
                            xTunnelBound=new BoundClass(lastRoom.xBound.max,(lastRoom.xBound.max+doorAdd));
                            zTunnelBound=new BoundClass((lastRoom.zBound.min+doorOffset),((lastRoom.zBound.min+doorOffset)+constants.ROOM_BLOCK_WIDTH));
                        }
                    ...
                    case constants.ROOM_SIDE_BOTTOM:
                        ...
                        if (tunnelMode!==this.TUNNEL_NONE) {
                            ...
                            doorOffset=this.findRandomBlockOffsetBetweenTwoBounds(lastRoom.xBound,xBound);
                            xTunnelBound=new BoundClass((lastRoom.xBound.min+doorOffset),((lastRoom.xBound.min+doorOffset)+constants.ROOM_BLOCK_WIDTH));
                            zTunnelBound=new BoundClass(lastRoom.zBound.max,(lastRoom.zBound.max+doorAdd));
                        }
            ...
            // add in tunnels and a light
            // if the tunnel is long
            
        if (tunnelMode!==this.TUNNEL_NONE) {
            this.addTunnelRoom(connectSide,tunnelMode,xTunnelBound,zTunnelBound);
            this.addTunnelLight(connectSide,tunnelMode,doorAdd,xTunnelBound,zTunnelBound);
        }
        ...
        if (tunnelMode!==this.TUNNEL_NONE) {
            ...
            // the newest room or tunnel to room
            // we use this mask to calculate ledges and other
            // outside wall hugging map pieces
        
        if (lastRoom!==null) {
            switch (tunnelMode) {
                case this.TUNNEL_SHORT:
                case this.TUNNEL_LONG:
                    lastRoom.maskEdgeGridBlockToBounds(xTunnelBound,zTunnelBound);
                    room.maskEdgeGridBlockToBounds(xTunnelBound,zTunnelBound);
                    break;
                default:
        ...
        if ((this.map.roomList.count()>=config.ROOM_PATH_COUNT) || (config.SIMPLE_TEST_MAP)) return;
            ...
            // compact mode has no tunnels
        
        if (config.MAP_DESIGN_TYPE===this.DESIGN_COMPACT) {
            tunnelMode=this.TUNNEL_NONE;
        }
        else {
            tunnelMode=(genRandom.randomPercentage(this.ROOM_LONG_TUNNEL_PERCENTAGE))?this.TUNNEL_LONG:this.TUNNEL_SHORT;
        }
        
            // next room in path
        
        this.buildMapRoomPath(room,tunnelMode);
    }
    ...
    buildRoomExtensionSingle(level,lastRoom,connectSide)
    {
        ...
        level=constants.ROOM_LEVEL_NORMAL;         // supergumba -- take all this away for outdoor maps
    ...
    buildMapPath()
    {
        ...
        this.buildMapRoomPath(null,this.TUNNEL_NONE);
