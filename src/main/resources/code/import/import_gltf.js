...
export default class ImportGLTFClass
{
    ...
    decodeMapMeshInformational(map,materialNode,meshNode,mesh,meshIdx)
    {
        ...
        if (value!==null) {
            ...
            map.music.setMusic(obj.name,((obj.loopStart===undefined)?0:obj.loopStart),((obj.loopEnd===undefined)?0:obj.loopEnd),false);
            return(this.MESH_INFORMATIONAL_REMOVE);
        }
        ...
        if (value!==null) {
            ...
            cubeClass=this.core.project.cubeClasses.get(obj.name);
            if (cubeClass===null) {
    ...
    decodeMesh(map,meshList,skeleton)
    {
        ...
        for (n=0;n!==meshesNode.length;n++) {
            ...
            for (k=0;k!==meshNode.primitives.length;k++) {
                ...
                if (this.getCustomProperty(materialNode,meshNode,'wsjsBump')!==null) mesh.bump=true;
    ...
    async import(map,meshList,skeleton,loadBitmapList)
    {
        let bitmap;
        ...
        if (skeleton!==null) {
            ...
            // put the bitmap cache into a loading list
            
        for (bitmap of this.bitmapCache.values()) {
            loadBitmapList.push(bitmap);
        } 
